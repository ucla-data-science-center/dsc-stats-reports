...
This report provides an in-depth analysis of the Data Science Center (DSC) activities at UCLA for 2024. It is designed for DSC team members, library management, and other stakeholders interested in understanding the growth, impact, and trends in DSC events, consultations, and DataSquad work over time. By combining multiple data sources—from workshop attendance records and one-on-one consultations to Trello project assignments and UCLA Dataverse metrics—this report presents a comprehensive view of our operational landscape.

### Objectives

The primary goals of this report are to:

-   **Monitor Growth:** Track changes in event attendance and departmental engagement.
-   **Assess Impact:** Examine trends in consultation services and direct interactions by the DataSquad team.
-   **Inform Decision-Making:** Provide actionable insights that can help guide future programming, resource allocation, and strategic planning.

### Data Sources and Methodology

The analysis is based on several key data sources:

- **Workshops Data:** Records of DSC events and attendance, capturing departmental participation. - - **Consultation Data:** Merged and standardized data on consultations, reflecting one-on-one interactions with researchers. 
- **Trello Data:** Project task information from the DSC Trello board, highlighting DataSquad activities. 
- **UCLA Dataverse:** Metrics on published datasets and files, offering insights into research data engagement.

Data cleaning and preprocessing have been performed using custom R scripts (`data_cleaning.R` and `text_analysis.R`). We employ the `tidyverse` for data manipulation and `ggplot2` for visualization, ensuring the insights are presented in a clear and accessible format.

### Report Structure

This document is organized into the following sections: 

1. **Events & Workshops:** Analyzes attendance trends and departmental participation over time. 
2. **Consultations:** Provides an overview of consultation services, including departmental and status breakdowns. 
3. **DataSquad Work Overview:** Summarizes direct interactions and task assignments managed via Trello. 
4. **Consulting Requests by Topic:** Utilizes text analysis to identify common themes in consultation requests. 
5. **Data Repositories & Infrastructure:** Explores trends in UCLA Dataverse usage, including dataset and file publications.

### Intended Audience

This report is intended for: 

- **DSC Team Members:** To monitor service growth and identify opportunities for operational improvements. 
- **Library and Academic Administration:** To gain insights into the impact of DSC services and support strategic planning. 
- **External Stakeholders:** To provide transparency regarding DSC activities and contributions to the broader academic community.
...
The DataSquad team engages in various activities, supporting the research data management efforts at UCLA. Their work can be categorized into direct interactions with patrons and assigned tasks from DSC staff consultants, often managed through Trello boards.
...
ggplot(yearly_interactions, aes(x = year, y = total_count)) +
  ...
  labs(title = "Total Direct Interactions by Year for DataSquad", x = "Year", y = "Number of Consults") +
  theme_minimal() +
  coord_flip()
```
...
In addition to direct interactions, DataSquad handles a significant number of project tasks assigned by the DSC team via our Trello board. These tasks include various research support activities, managed and tracked by DSC staff consultants. The following chart provides an overview of the tasks handled by DataSquad over the past year:
