...
df_lariac_growth = aws_processing.fetch_s3_historical_growth('lariac-backup', days=180)
...
::: {.g-col-6}
### LARIAC Growth (180 Days)
```{python}
#| echo: false
if not df_lariac_growth.empty:
    plt.figure(figsize=(8, 4))
    plt.plot(df_lariac_growth['date'], df_lariac_growth['size_gb'], color='#d62728', linewidth=2)
    plt.fill_between(df_lariac_growth['date'], df_lariac_growth['size_gb'], alpha=0.2)
    plt.title('LARIAC Storage Growth (GB)')
    plt.grid(True, alpha=0.3)
    plt.show()
else:
    print("Growth data for LARIAC unavailable.")
```
:::
