...
stopifnot(sum(full_combined$attendance_count[full_combined$institution == "UCLA" & !is.na(full_combined$organizational_parent)], na.rm = TRUE) == 836L)
stopifnot(sum(full_combined$attendance_count[full_combined$institution == "UCLA" & !is.na(full_combined$standardized_department) & is.na(full_combined$organizational_parent)], na.rm = TRUE) == 31L)
